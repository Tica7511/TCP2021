<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/AdminMasterPage.master" AutoEventWireup="true" CodeFile="classview.aspx.cs" Inherits="Admin_classview" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            /*getForumsData(0);*/
            $("#txt_embedcode").attr("placeholder", '嵌入碼要像此範例: <iframe src="..."></iframe> ');
					...
					if ($(data).find("Error").length > 0) {
						...
						if ($(data).find("data_item").length > 0) {
                            $(data).find("data_item").each(function (i) {       
                                $("#txt_name").val($(this).children("C_Name").text().trim());
                                $("#txt_embedcode").val($(this).children("EncodeEmbedCode").text().trim());
                                $("#txt_teachername").val($(this).children("C_TeacherName").text().trim());
                                $("#txt_opdate").val(getDate($(this).children("C_OpenDate").text().trim()));
                                $("#txt_ophh").val(getHH($(this).children("C_OpenDate").text().trim()));
                                $("#txt_opmm").val(getMM($(this).children("C_OpenDate").text().trim()));
                                $("#txt_csdate").val(getDate($(this).children("C_CloseDate").text().trim()));
                                $("#txt_cshh").val(getHH($(this).children("C_CloseDate").text().trim()));
                                $("#txt_csmm").val(getMM($(this).children("C_CloseDate").text().trim()));
                                $("#txt_sort").val($(this).children("C_Sort").text().trim());
                                $("#Csort").val($(this).children("C_Sort").text().trim());
                                if ($(this).children("C_Status").text().trim() == 'A')
            ...
            $.ajax({
				...
				success: function (data) {
					if ($(data).find("Error").length > 0) {
						...
						if ($(data).find("data_item").length > 0) {
                            $(data).find("data_item").each(function (i) {       
                                $("#txt_sort").append($("<option></option>").attr("value", $(this).children("C_Sort").text().trim()).text($(this).children("C_Sort").text().trim()));
							});
                        }
					}
				}
