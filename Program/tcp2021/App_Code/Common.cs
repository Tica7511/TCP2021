...
public class Account
{
	public static void ExecSignIn(string account,string pw)
    {
        ...
        if (dt.Rows.Count > 0)
        {
			LogInfo.id = dt.Rows[0]["M_ID"].ToString();
			LogInfo.mGuid = dt.Rows[0]["M_Guid"].ToString();
			LogInfo.group = dt.Rows[0]["M_Group"].ToString();
			LogInfo.account = dt.Rows[0]["M_Account"].ToString();
			LogInfo.name = dt.Rows[0]["M_Name"].ToString();
			LogInfo.birthDay = dt.Rows[0]["M_BirthDay"].ToString();
			LogInfo.email = dt.Rows[0]["M_Email"].ToString();
			LogInfo.introduction = dt.Rows[0]["M_Introduction"].ToString();
			LogInfo.competence = dt.Rows[0]["M_Competence"].ToString();
		}
        else
        {
			throw new ApplicationException("Invalid account or password");
        }
    }
}
#endregion
